...
void put_preamble(void)
{
    ...
    if (needed) {
	fprintf(output_file, "%% Generated by euktopst 1.5.0\n");
	if (NEQL(scale, 1)) {
	    fprintf(output_file, "\\bgroup\n\\psset{unit=%7.4fcm}\n", scale);
	    scaled = 1;
	}
	fprintf(output_file, "\\pspicture*(%7.4f,%7.4f)(%7.4f,%7.4f)\n",
...
void draw_vector(void)
{
    ...
    put_local();
    fprintf(output_file, "{->}(%7.4f,%7.4f)(%7.4f,%7.4f)\n",
...
void draw_line(void)
{
    ...
    if (i == 2) {
	fprintf(output_file, "\\psline{c-c}");
	put_local();
	fprintf(output_file, "(%7.4f,%7.4f)(%7.4f,%7.4f)\n", x[0], y[0], x[1], y[1]);
    }
}
...
void draw_circle(void)
{
    ...
    put_local();
    fprintf(output_file, "(%7.4f,%7.4f){%7.4f}\n", c->x, c->y, c->r);
}
			...
			double c, double s)
{
    ...
    fprintf(output_file, "\\pspolygon");
    put_local();
    for (t = f, n = 0; t <= g; t += CONIC_DRAW_STEP, n++) {
			 ...
			 double f, double g, double c, double s)
{
	...
	fprintf(output_file, "\\pspolygon");
	put_local();
	for (t = i, n = 0; t <= j; t += CONIC_DRAW_STEP, n++) {
...
void draw_string(_param p)
{
    ...
    fprintf(output_file, "\\uput[%7.4f](%7.4f,%7.4f){", a, x, y);
    put_font();
...
void label_angle(void)
{
    ...
    void draw_arc(const char* arrow)
    {
	fprintf(output_file, "  \\psarc%s(%7.4f,%7.4f){%7.4f}{%7.4f}{%7.4f}\n",
		arrow, B->x, B->y, r, a, b);
    }
	...
	case SIMPLE:
		...
		fprintf(output_file, "  \\Dot{%7.4f}{%7.4f}{%7.4f}\n",
			B->x + x1 + x2, B->y + y1 + y2, SIZE(.05));
	    }
	    if (local_dec == DASHED) {
		fprintf(output_file, "  \\Dash{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}\n",
	...
	case DOUBLE:
		...
		draw_arc("");
		t = 8/local_size;
		fprintf(output_file, "  \\DoubleDash{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}\n",
			B->x, B->y, r - s, r + s, (a + b)/2 - t/2, t);
	    } else {
		fprintf(output_file, "  \\DoubleArc{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}\n",
	...
	case TRIPLE:
		...
		draw_arc("");
		t = 8/local_size;
		fprintf(output_file, "  \\TripleDash{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}\n",
			B->x, B->y, r - s, r + s, (a + b)/2 - t, t);
	    } else {
		fprintf(output_file, "  \\TripleArc{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}\n",
			B->x, B->y, r - s, r, r + s, a, b);
	    }
	    break;
	case RIGHT:
	    set_xy(SIZE(.35));
	    fprintf(output_file, "  \\Right{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}{%7.4f}\n",
...
void label_point(_param p)
{
    ...
    fprintf(output_file, "\\uput[%7.4f](%7.4f,%7.4f){", a, O->x, O->y);
    put_font();
    fprintf(output_file, "$%s$}\n", s->symbol);
}
