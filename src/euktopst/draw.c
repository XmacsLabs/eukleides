...
void put_preamble(void)
{
    ...
    if (needed) {
	fprintf(output_file, "%% Generated by euktopst 1.5.1\n");
	if (NEQL(scale, 1)) {
...
void draw_line(void)
{
	...
	fprintf(output_file, "\\psline");
	put_local();
	fprintf(output_file, "{c-c}(%7.4f,%7.4f)(%7.4f,%7.4f)\n",
		x[0], y[0], x[1], y[1]);
    }
}
...
void draw_string(_param p)
{
    ...
    fprintf(output_file, "\\rput(%7.4f,%7.4f){", x, y);
    put_font();
...
void label_point(_param p)
{
    ...
    fprintf(output_file, "\\rput(%7.4f,%7.4f){", O->x + r*Cos(a), O->y + r*Sin(a));
    put_font();
    fprintf(output_file, "$%s$}\n", s->symbol);
}
